...
const PronunciationView: React.FC<PronunciationViewProps> = ({ words, studyProgress, onGoalUpdate }) => {
    ...
    const callGeminiForFeedback = async (userTranscript: string) => {
        ...
        try {
            ...
            const prompt = `As an English pronunciation coach, evaluate a user's pronunciation.
- The target word is: "${targetWord.english}" (phonetics: /${targetWord.pronunciation}/).
- The user said: "${userTranscript}".

Please provide your evaluation in a valid JSON object with two keys:
1. "score": An integer from 0 to 100 on how accurate the pronunciation was.
2. "comment": A short, constructive feedback in Vietnamese, pointing out one main area for improvement if needed, or praising them if it's good.

Example for a good attempt: {"score": 90, "comment": "Tuyệt vời! Bạn đã phát âm rất gần với âm chuẩn."}
Example for a mistake: {"score": 65, "comment": "Khá tốt! Hãy chú ý phát âm rõ hơn âm /t/ ở cuối từ nhé."}`;
                    ...
                    <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-200 animate-fade-in">
                        ...
                        <div className="pt-4 border-t">
                            ...
                            {feedback && (
                                <div className="mt-2">
                                    <div className="flex justify-between items-center">
                                        <p className="text-md text-slate-800 font-medium p-3 bg-blue-50 rounded-md border border-blue-200 flex-grow">{feedback.comment}</p>
